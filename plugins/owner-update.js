...
let handler = async (m, { conn }) => {
  await conn.reply(m.chat, '🌪️ 𝒑𝒓𝒐𝒄𝒆𝒔𝒂𝒏𝒅𝒐 𝒔𝒐𝒍𝒊𝒄𝒊𝒕𝒖𝒅 𝒅𝒆 𝒂𝒄𝒕𝒖𝒂𝒍𝒊𝒛𝒂𝒄𝒊𝒐𝒏...', m, rcanalw);

  exec('git pull', (err, stdout, stderr) => {
    if (err) {
      conn.reply(m.chat, `⚠️ Error: No se pudo realizar la actualización.\nRazón: ${err.message}`, m, rcanalden2);
      return;
    }
    ...
    if (stdout.includes('Already up to date.')) {
      conn.reply(m.chat, '🔥 𝒆𝒍 𝒃𝒐𝒕 𝒚𝒂 𝒆𝒔𝒕𝒂 𝒂𝒄𝒕𝒖𝒂𝒍𝒊𝒛𝒂𝒅𝒐.', m, rcanalden2);
    } else {
